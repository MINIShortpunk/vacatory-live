...
function renderFunding() {
  ...
  const canonicalVisaSummary = String(
    state.chamber?.uk_visa_summary || ""
  ).trim();

  const visaEntries = canonicalVisaSummary
    ? [{
        title: "Provider-level UK visa finding",
        value: canonicalVisaSummary,
        sourceUrl: state.chamber?.uk_visa_source_url || ""
      }]
    : chamberUkStatementEntries(
        ukOpportunities,
  ...
  const items = [
    chamberUkFirmStatement(
      ...
      visaEntries,
      state.chamber?.uk_visa_source_url ||
